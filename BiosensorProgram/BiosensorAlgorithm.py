...
#parser.read ("/home/pi/Documents/OnlineBiosensor/Configuration/biosensorconfig.ini") #location of biosensor configuration/parameters for floating sensor #raspberry pi
#parser2.read ("/home/pi/Documents/OnlineBiosensor/Configuration/biosensorconfig2.ini") #location of biosensor configuration/parameters for flow through sensor #raspberry pi
...
#the filepaths are applicable for Windows, NRC Laptop
##configfile1path = r'C:\\Users\adekunlea\Documents\OnlineBiosensor\Configuration\biosensorconfig.ini'
##configfile2path = r'C:\\Users\adekunlea\Documents\OnlineBiosensor\Configuration\biosensorconfig2.ini'
##BiosensingDataFolder = r'C:\Users\adekunlea\Documents\OnlineBiosensor\BiosensingData'
##BODExcelLocation = r'C:\Users\adekunlea\Documents\OnlineBiosensor\BiosensingData\BOD_Calibration_Data - Copy - Copy.xlsx' #Windows. need to change for Raspberry Pi

#these are the filepaths for Debian/Raspberry Pi as of July 6, 2020 
configfile1path = r'/home/pi/Documents/OnlineBiosensor/Configuration/biosensorconfig.ini'
configfile2path = r'/home/pi/Documents/OnlineBiosensor/Configuration/biosensorconfig2.ini'
BiosensingDataFolder = r'/home/pi/Documents/OnlineBiosensor/BiosensingData'
BODExcelLocation = r'/home/pi/Documents/OnlineBiosensor/BiosensingData/Calibration_Data.xlsx'
...
d.configIO(FIOAnalog = 71) #(71=1000111 binary, so FIO0-FIO2 is set to analog, FI06=analog and the rest to digital...()
...
##        with open ('/home/pi/Documents/OnlineBiosensor/Configuration/biosensorconfig.ini', 'w') as configfile: #RPi
...
##        if not os.path.exists('/home/pi/Documents/OnlineBiosensor/BiosensingData'): #RPi
...
##        with open ('/home/pi/Documents/OnlineBiosensor/Configuration/biosensorconfig.ini', 'w') as configfile: #Pi
        with open (configfile1path, 'w') as configfile: #Windows
            parser.write(configfile)
        configfile.close()
...
##                        if not os.path.exists('/home/pi/Documents/OnlineBiosensor/BiosensingData'):  #Pi
##                            os.makedirs('/home/pi/Documents/OnlineBiosensor/BiosensingData') #create folder for documents if it doesnt exist
##                        os.chdir('/home/pi/Documents/OnlineBiosensor/BiosensingData') #change directory to this new folder and save files here
                        ...
                        filename=filename
##                        os.chmod("/home/pi/Documents/OnlineBiosensor/BiosensingData", 0777) #make folder write and readable for all #Pi
                        ...
                        'Est_COD', 'Y_calculated', 'Vmax/Vmin', 'OCVwait', 'Vminwait', 'steadystaterepeatocv', 'steadystaterepeatVmin', 'pumprate']
...
##
                                    ...
                                    if filesaveflag == 1:
                                        ...
                                        datastr = (timeofsave +' ' + str(Vmax_Est)+' ' + str(Area_curve)+
                                                  ...
                                                  ' '+ str(Sensoroutput)+' '+str(ocvwait)+' '+str(vminwait)+' '+str(steadyrepeat)+' '+str(steadyrepeatvmin)+' '+ str(targetpumprate))
...
##                        os.chmod("/home/pi/Documents/OnlineBiosensor/BiosensingData", 0777) #make folder write and readable for all
                        ...
                        headers2=['Date', 'Time', 'Vmax_Est', 'Area', 'OCV_time(s)','Tolerance', 'Vmin', 'alpha1','Vmin_Est','prev_discharge_time','DischargeArea','Ambient_Temperature','Vmax/Vmin','OCVwait','Vminwait','steadystaterepeatocv','steadystaterepeatVmin','pumprate']
                        headers2=' '.join(headers2)
                        save_file2.write(headers2 +'\n') #write headers to file
                        save_file2.close()
...
##                                        y4limit2 = max(vmintimevec2) + (0.5*max(vmintimevec2))
                                    ...
                                    if filesaveflag_2==1:
                                        ...
                                        datastr2= (timeofsave2 +' ' + str(Vmax_Est2)+' ' + str(Area_curve2)+
                                                  ' ' + str(OCV_time2)+' ' + str(tolerance2)+' '+str(Vmin2)+' '+str(alpha1_2)+' '+str(Vmin_Est2)+' '+str(discharge_time2)+' '+str(Disarea_curve2)+' '+str(tempV2)+' '+str(Sensoroutput2)+' '+str(ocvwait2)+' '+str(vminwait2)+' '+str(steadyrepeat2)+' '+str(steadyrepeatvmin2)+str(pumpcontrolon2))
...
##                os.chdir('/home/pi/Documents/OnlineBiosensor/BiosensingData') #change directory to this new folder and save files here
    ...
    def labjackdataacqusition():
        ...
        while True:
            ...
            if flag==1:
                if state==0:
                    d.getFeedback(u3.BitStateWrite(3, 1)) # Set FIO4 to output high therefore connecting resistor & discharging
                else:
                    d.getFeedback(u3.BitStateWrite(3, 0)) # Set FIO4 to output low therefore open circuit mode

            elif flag==0:
                d.getFeedback(u3.BitStateWrite(3, 0)) #SET FI04 to output low to disconnect resistor #OFF MODE
            ...
            if flag_2==1:
                if state2==0:
                    ...
                    d.getFeedback(u3.BitStateWrite(5, 0)) # Set FIO5 to output low, therefore open circuit mode

            elif flag_2==0:
                d.getFeedback(u3.BitStateWrite(5, 0)) #SET FI05 to output low to disconnect resistor #OFF MODE
            ...
            while read==1:
                ...
                ainValuetemp = d.getAIN(0) #get the analog input value from channel 1, hence AIN(1)-FLOATING
                ainValue2temp = d.getAIN(1) #get the analog input value from channel 2, hence AIN(2)-FLOW THROUGH
                tempvaluesstoreclosed=tempvaluesstoreclosed+[tempVtemp]
                mfcvaluesstoreclosed=mfcvaluesstoreclosed + [ainValuetemp]
                mfcvaluesstoreclosed2=mfcvaluesstoreclosed2 + [ainValue2temp]
                time.sleep(0.05)
                acquisframe=len(tempvaluesstoreclosed) #to acquire 5 readings and average
                if acquisframe>10:      #acquire at least 10 readings and average
                    read=0
                    valuesready=1
                    tempV=np.mean(tempvaluesstoreclosed)
                    ainValue=(np.mean(mfcvaluesstoreclosed))*1000
                    ainValue2=(np.mean(mfcvaluesstoreclosed2))*1000
                    mfcvaluesstoreclosed=[]
                    mfcvaluesstoreclosed2=[]
                    tempvaluesstoreclosed=[]
    ...
    t1=threading.Timer(0.5,floating,args=(q,)) #executes maincode
    t2=threading.Timer(1,pump_control_2) #executes pump control
    t3=threading.Timer(0.5,flowthrough,args=(q2,)) #executes maincode
    t4=threading.Timer(0.001,labjackdataacqusition) #executes maincode
