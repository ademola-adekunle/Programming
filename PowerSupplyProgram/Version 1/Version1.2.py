...
#FILENAMES & LOCATIONS
inifile_location = r'C:\Users\ademo\github\Programming\PowerSupplyProgram\Version 1\INI
inifile = r'C:\Users\ademo\github\Programming\PowerSupplyProgram\Version 1\INI\psSettings
...
try:
   ...
   ps1Aval = True
except Exception:   #add text in bold to display that PS1 is not present
   pass

try:
   ps2 = KoradSerial(ps2serialPort) # handle def
   ps2Aval = True
except Exception:   #add text in bold to display that PS2 is not present
   pass

#______________________________________________________________________________
# EXTRACT VALUES FROM INIFILE OR CREATE ONE IF NOT PRESENT
# Creating a folder for the INI file (startup)
if not os.path.exists(inifile_location):
   os.makedirs(inifile_location)    # Folder and location for INI file
   os.chdir(inifile_location )       # change directory to this if needed? INI file can be in another directory

if not os.path.exists(inifile):
   cfgfile = open(inifile,'w') #INI file creation. I would take this from the final code. This is just for the test
...
ps2Voltage     = float(parser.get("Settings", 'pS2Voltage'))     # Initial voltage definition for PS2
ps2maxCurrent  = float(parser.get("Settings", 'ps2Current'))     # Initial current definition for PS2
...
class simpleapp_tk(Tkinter.Tk):
   ...
   def Settings(self):  # Defines what SETTINGS button does
     ...
     # ------------------------------------------------------------------------
      ...
      def on_closing():
         ...
         cfgfile = open(inifile_location,'w') #INI file creation. I would take this from the final code. This is just for the test
         parser = SafeConfigParser()

         parser.read("C:/Python2716/PS Development/JC_Code/INI/psSettings.ini")
         parser.add_section('Settings')
         ...
         with open("C:/Python2716/PS Development/JC_Code/INI/pssettings.ini",'wb') as configfile:
            parser.write(configfile)
         configfile.close()
   ...
   def write_ini(self):
      ...
      global runPS1
      global runPS2
      cfgfile = open("C:/Python2716/PS Development/JC_Code/INI/psSettings.ini",'w') #INI file creation. I would take this from the final code. This is just for the test
      parser = SafeConfigParser()
      parser.read("C:/Python2716/PS Development/JC_Code/INI/psSettings.ini")
      parser.add_section('Settings')
      parser.set('Settings', 'ps1Voltage', ps1Vini)
...
if not os.path.exists('/Python2716/PS Development/JC_Code/psDatalogging'):
   os.makedirs('/Python2716/PS Development/JC_Code/psDatalogging') #create folder for documents if it doesnt exist
   os.chdir('/Python2716/PS Development/JC_Code/psDatalogging') #change directory to this new folder and save files here
...
def maincode():
   ...
   while True:
      ...
      if not dAcqFlag:
         dAcqTimer = 0
      # 2 different communications attempts are necessary, so if 1 power supply does not work,
