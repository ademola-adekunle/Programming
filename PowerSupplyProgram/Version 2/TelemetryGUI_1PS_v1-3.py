## For first run on this machine, please follow Steps 1 to 3. P
...
# Step 4: Open and run setup.py in the main program #OR manually place the korad file in the site packages folder e..g (C:\Users\adekunlea\AppData\Local\Programs\Python\Python38\Lib\site-packages
...
def INI_write(): # function to write an INI file
    global ps1Voltage, ps1CurrentMax, ps1VoltageMax, runPS1, dAqInterval, dAqON, cc_advset, cv_advset, ovp_advset, ocp_advset, koradports, ports
    cfgfile = open(r"C:\Users\adekunlea\Documents\Programming\PowerSupplyProgram\Version 2\psSettings.ini",'w') #INI file creation. I would take this from the final code. This is just for the test
    parser = ConfigParser()
    ...
    #     print(COMs)
    #     try:
    #         parser.set('COM Ports', 'Korad port', str(COMs))
    #
    #     except Exception:
    #             pass
    ...
    try: # exception catch in case there are no COM ports recognized
        for i in len(ports):
            parser.set('COM Ports', 'Korad port #%i' %i, str(ports[i]))
...
def INI_read(): # function to read an INI file
    global ps1Voltage, ps1CurrentMax, ps1VoltageMax, runPS1, dAqInterval, dAqON, cc_advset, cv_advset, ocp_advset, ovp_advset, koradports
    ...
    #You could use a dict to get all the values from the COM Ports, then extract if anything present
    #storedports = dict(parser.items('COM Ports'))
    #list = (storedports.values())
...
if not os.path.exists(r"C:\Users\adekunlea\Documents\Programming\PowerSupplyProgram\Version 2\INI\psSettings.ini"):
    ...
    if ports: # if there are ports detected #
        ...
        for i in range(len(ports)):
            try:
                koradports = KoradSerial(str(ports[i])) #send the communication to the serial port anyway, what it should do is attempt to receive info or send IDN? check documentation

            except Exception:
                koradports = []
                pass
            koradonport = korandonport.append(str(ports[i]) #or something like this to prevent list the number of korad
    #koradports
    ...
    if ports: # if there are ports detected
        for i in range(len(ports)):
            try:
                koradports = KoradSerial(str(ports[i]))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.plotsetButton.clicked.connect(self.check_advset) # should be on_plotset_button_clicked, timer for testing
        ...
        # Add Title
        self.graphWidget.setTitle(u"MEC Data", color="k", size="12pt")
        # Add Axis Labels
    ...
    def get_telem(self):
        try:
            ...
            os.chdir(r"C:\Users\adekunlea\Documents\Programming\PowerSupplyProgram\Version 2\Data_Logs")
